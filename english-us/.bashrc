...
alias down='echo && echo Shutdown System && sudo shutdown -h 0'
...
checkosversion() {
    ...
    if [ "$os_ver" -ge 13 ] 2>/dev/null; then
        ...
        # ========================================================
        # ********************************************
        local_lang_us # Set language to English (US)
        # ********************************************
        local lang_changed=$?
...
piholeinstall() {
    ...
    if [[ "$confirm" == "yes" ]]; then
        ...
        # Default .toml Datei erzeugen für das unattended Setup von Pi-hole
        # ********************************
        defaulttomlfile -phi || return 1
        # ********************************
...
defaulttomlfile() {
 ...
 local param="$1"
   if [[ -z "$param" ]]; then
   printf "\n⚠️ The function 'defaulttomlfile' cannot be called in this way — Aborting! \n\n"
    return 0
   fi
...
if [[ "$param" == "-phi" ]]; then
 ...
 # Standard-Konfiguration für unattended Setup
...
sleep 2
   ...
   if [[ -s "/etc/pihole/pihole.toml" ]]; then
     return 0
   else
      printf "\n❌ Error: The file /etc/pihole/pihole.toml could not be created - Aborting!\n\n"
     return 1
   fi
else
 printf "\n❌ Error: Invalid parameter – Aborting!\n\n"  
fi
}
      ...
      if [[ "$param" != "-ub" ]]; then
       clear # Clear screen
        while true; do
            clear
            printf "=========================================\n"
            printf "🌐 Upstream DNS Server Selection\n"
            printf "=========================================\n"
            printf "1) Cloudflare\n"
            printf "2) OpenDNS\n"
            printf "3) Quad9 (filtered, DNSSEC)\n"
            printf "4) Cloudflare and OpenDNS\n"
            ...
            if [[ "$choice" =~ ^[1-8]$ ]]; then
                ...
                printf "\n❌ Invalid input! Please choose a number from 1 to 8.\n"
                sleep 3
            fi
        done
      else
        choice=8  # 'UNBOUND' pre-selection
      fi

        # ----- DNS list assignment -----
        if [[ "$choice" == 8 ]]; then
            # If UNBOUND: Skip variables for external providers
            if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
               dns_list='[ "127.0.0.1#5335", "::1#5335" ]' # IPv4 and IPv6
            else
               dns_list='[ "127.0.0.1#5335" ]' # IPv4 only
            fi
        else
            # Load local variables ONLY for external providers
        ...
        if [[ "$choice" == 8 ]]; then
             installunbound -phi # Install 'UNBOUND' now
        elif (( choice > 3 )); then
            printf "\n\n⚙️ Setting up the selected upstream servers...\n"
        else
            printf "\n\n⚙️ Setting up the selected upstream server...\n"
        fi
         # Upstream servers are being created
         ...
         # Prepare entry for TOML

         local host_entry="[ \"$pi_ip $pi_hostname.lan\", \"127.0.0.1 pihole.lan\" ]"

         local interface=$(nmcli -t -f DEVICE,STATE device status | grep -E ":connected|:verbunden" | head -n 1 | cut -d: -f1)
         local sub_net=$(ip r | grep -v default | grep "proto kernel" | awk '{print $1}' | head -n 1)
         local gw_ip=$(ip r | grep default | awk '{print $3}')
         local domain=$(grep -i '^search' /etc/resolv.conf | awk '{print $2}' | xargs)
         [ -z "$domain" ] && domain="lan"
         local phrevServers="[\"true,$sub_net,$gw_ip,$domain\"]"
        
         sudo pihole-FTL --config dns.interface "$interface" > /dev/null 2>&1
         ...
         sudo pihole-FTL --config dns.expandHosts true > /dev/null 2>&1
         sudo pihole-FTL --config dns.hosts "$host_entry" > /dev/null 2>&1 
         sudo pihole-FTL --config dns.revServers "$phrevServers" > /dev/null 2>&1
         sudo pihole-FTL --config dhcp.active false > /dev/null 2>&1
         sudo pihole-FTL --config database.DBinterval 5 > /dev/null 2>&1
         if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
         sudo pihole-FTL --config resolver.resolveIPv6 true > /dev/null 2>&1
         else
         sudo pihole-FTL --config resolver.resolveIPv6 false > /dev/null 2>&1
         fi
         sleep 3
         printf "\n⚙️ Pi-hole optimization completed...\n\n"
        printf "\n🔄 Restarting Pi-hole FTL - Please wait...\n\n"      
        sudo systemctl restart pihole-FTL > /dev/null 2>&1
        sleep 3
        printf "\n✅ DNS Upstream server configured and optimizations completed successfully.\n\n"
    else
...
installunbound() {
    ...
    # Pre-check: Is Pi-hole installed?
    if ! command -v pihole >/dev/null 2>&1; then
    clear # # Clear screen
    printf "\n\n⚠️ Attention: Please install Pi-hole first and select 'UNBOUND' as your upstream DNS server!\n"
    printf "\n\n⌨️ Press any key to continue...\n\n"
    read -n 1 -s -r
    piholeinstall # Install Pi-hole now
    return 1
    fi

    
    if dpkg -l | grep -q "^ii  unbound "; then
        printf "\n\n\nℹ️ 'UNBOUND' is already installed on this system.\n\n"
        return 0
    fi

    local param="$1"
    if [[ -z "$param" ]]; then
    upstreamdns -ub # Standard: UNBOUND choice
    return 0
    fi

    local ub_conf="/etc/unbound/unbound.conf.d/pi-hole.conf"

        clear # Clear screen
        printf "\n\n🚀 Starting 'UNBOUND' installation...\n\n"
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" unbound

        if dpkg -l | grep -q "^ii  unbound "; then
            printf "\n⚙️  Creating 'UNBOUND' configuration for Pi-hole...\n\n"
            # --- IMPORTANT: Prevent port conflict with default config ---
            sudo mv /etc/unbound/unbound.conf /etc/unbound/unbound.conf.bak 2>/dev/null
            sudo bash -c 'echo "include-toplevel: \"/etc/unbound/unbound.conf.d/*.conf\"" > /etc/unbound/unbound.conf'
            ...
            if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
                ...
                ipv6_access="access-control: ::1 allow"
                ipv6_do="yes"
            fi
            sudo bash -c "cat <<EOF > $ub_conf
server:
    interface: 127.0.0.1
    $ipv6_interface
    port: 5335
...
remote-control:
    control-enable: yes
EOF"
            printf "\n🌐 Downloading root hints (Root Server List) for 'UNBOUND'...\n\n"
            # URL for UNBOUND servers:
            sudo wget https://www.internic.net/domain/named.root -qO /var/lib/unbound/root.hints

            printf "\n🔧 Setting permissions and preparing startup...\n\n"
            sudo chown -R unbound:unbound /var/lib/unbound
            sudo chmod 755 /var/lib/unbound
            sudo systemctl reset-failed unbound > /dev/null 2>&1
            sudo unbound-control-setup > /dev/null 2>&1
            
            printf "\n🔄 Restarting 'UNBOUND' service...\n\n"
            # Clear main config and point to .d/ folder only
            sudo bash -c 'echo "include-toplevel: \"/etc/unbound/unbound.conf.d/*.conf\"" > /etc/unbound/unbound.conf'
            # Stop and disable socket if present (often blocks port 53)
            sudo systemctl stop unbound.socket > /dev/null 2>&1
            sudo systemctl disable unbound.socket > /dev/null 2>&1
            sudo systemctl enable --now unbound > /dev/null 2>&1
            sudo systemctl restart unbound > /dev/null 2>&1

            # --- Pi-hole v6 AUTOMATION ---
            printf "\n💉 Configuring Pi-hole v6 upstream DNS to 'UNBOUND'...\n\n"
            local pi_upstreams="127.0.0.1#5335"
            [[ "$ipv6_do" == "yes" ]] && pi_upstreams="127.0.0.1#5335,[::1]#5335"

            if command -v pihole-FTL >/dev/null 2>&1; then
                sudo pihole-FTL --config dns.upstreams "$pi_upstreams" > /dev/null 2>&1
                sudo pihole restartdns reload > /dev/null 2>&1
                printf "✅ Pi-hole v6 has been automatically switched to 'UNBOUND'.\n"
            fi

            setunboundcronjob # Cronjob for updating 'UNBOUND' root hints

            # --- FINAL MESSAGE ---
            printf "\n====================================================\n"
            printf "✅ UNBOUND SUCCESSFULLY INSTALLED AND CONFIGURED\n"
            printf "====================================================\n"
            if [[ "$ipv6_do" == "yes" ]]; then
                printf "📍 IPv4 Address: 127.0.0.1#5335\n"
                printf "📍 IPv6 Address: [::1]#5335\n"
                printf "🛡️ DNSSEC:       Enabled\n"
            else
                printf "📍 IPv4 Address: 127.0.0.1#5335\n"
                printf "🛡️ DNSSEC:       Enabled\n"
            fi
            ...
            read -n 1 -s -r
            clear # Clear screen
            checkunbound # Check 'UNBOUND'-Installation
        else
...
setunboundcronjob() {
    ...
    local script_path="/home/$USER/unbound_hints_update.sh"
    local log_path="/home/$USER/unbound_hints_update.log"
...
date > "$log_path" 2>&1
printf "🔄 Updating 'UNBOUND' Root Hints...\n" >> "$log_path"
sudo wget https://www.internic.net/domain/named.root -qO /var/lib/unbound/root.hints >> "$log_path" 2>&1
sudo systemctl reload unbound >> "$log_path" 2>&1
printf "✅ 'UNBOUND' Root Hints update completed.\n" >> "$log_path"
EOF

    chmod +x "$script_path" # Make script executable

    # Add Cronjob: Every 1st of January (1) and July (7) at 03:30 AM
    local cron_job="30 3 1 1,7 * $script_path"
    (crontab -l 2>/dev/null | grep -vF "$script_path"; echo "$cron_job") | crontab -
    printf "\n📅 Cronjob created: 'UNBOUND' Root Hints will be updated every 6 months (Jan & Jul).\n"
}
...
checkunbound() {
  ...
  if ! command -v pihole >/dev/null 2>&1; then
   ...
   return 1
    ...
    if ! dpkg -l | grep -q "^ii  unbound "; then
      clear # # Clear screen
      printf "\n\n⚠️ 'UNBOUND' is NOT installed yet!\n\n"
      printf "\n⚠️ Attention: If 'UNBOUND' is installed, Pi-hole will run through this service!\n\n\n"
      read -p "❓ Should 'UNBOUND' be installed now? (yes/no): " ubconfirm
      if [[ "$ubconfirm" == "yes" ]]; then
      upstreamdns -ub # Install UNBOUND
      else
      printf "\n\n"
      return 1
      fi
         ...
         clear # Clear screen
         sudo unbound-control flush pi-hole.net > /dev/null 2>&1 # Flush pi-hole.net from cache 
            # --- LIVE RECURSION TEST ---
            printf "\n\n\n🔍 Testing DNS resolution via 'UNBOUND' (Before / After)...\n"
            # 1. Test: Recursion (Internet query)
            printf "\n\n🔍 1. IPv4 Query (Directly from the Internet):\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @127.0.0.1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ ERROR: 'UNBOUND' is not responding on port 5335! (IPv4)\n"
            fi
            printf "====================================================\n"
            sleep 2 # Short pause for visual effect
            # 2. Test: Caching (Local query)
            printf "\n\n🔍 2. IPv4 Query from 'UNBOUND' cache:\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @127.0.0.1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ ERROR: IPv4 DNS query via 'UNBOUND' failed!\n"
            fi
            printf "====================================================\n\n\n"
            printf "\n🏁 'UNBOUND' IPv4 tests completed. Press any key to continue...\n"
            read -n 1 -s -r
            if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
            clear # Clear screen
            printf "\n\n\n🔍 Testing IPv6 DNS resolution via 'UNBOUND' (Before / After)...\n\n"
            sudo unbound-control flush pi-hole.net > /dev/null 2>&1 # Flush pi-hole.net from cache
            # 1. Test: Recursion (Internet query)
            printf "\n\n🔍 1. IPv6 Query (Directly from the Internet):\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @::1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ ERROR: 'UNBOUND' is not responding on port 5335! (IPv6)\n"
            fi
             printf "====================================================\n\n"
            sleep 2 # Short pause for visual effect
            # 2. Test: Caching (Local query)
            printf "\n\n🔍 2. IPv6 Query from 'UNBOUND' cache:\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @::1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ ERROR: IPv6 DNS query via 'UNBOUND' failed!\n"
            fi
            printf "====================================================\n\n\n"
            printf "\n🏁 'UNBOUND' IPv6 tests completed. Press any key to continue...\n"
            read -n 1 -s -r
            fi
            clear # Clear screen
            printf "\n\n\n"
...
setstaticip() {
    ...
    if [[ -f "$skip_file" ]]; then
        printf "\n\n⚠️ ATTENTION: The User has opted for permanent DHCP !\n\n"
    ...
    if [[ "$sip_antwort" == "yes" ]]; then
        ...
        printf "\n⌨️ Press any key to continue...\n\n"
        read -n 1 -s -r
        printf "\n\n🚀 Restarting in 5 seconds to switch to the new IP addresses - Please wait...\n\n"
        sleep 5
        sudo reboot
        exit 0
...
local_lang_us() {
    ...
    # --- CHECK: Is English (US) already active? ---
    local kbd_check=0
    local lang_check=0
...
EOF
        ...
        # Set timezone (Optional: changed to UTC or keep as needed. Here set to UTC for English (US)
        if [[ "$(cat /etc/timezone 2>/dev/null)" != "UTC" ]]; then
            sudo timedatectl set-timezone America/New_York 2>/dev/null
        fi
        
        printf "\n✅ System language and keyboard successfully switched to English (US).\n\n"
        return 1
    else
        printf "\nℹ️ System language and keyboard are already set to English (US).\n\n"
        return 0
    fi
}
