...
#            (DE)
#          Mai 2026
# Version 3.0 von Mario Ammerschuber
# -----------------------------------
...
piholeinstall() {
    ...
    if [[ "$confirm" == "ja" ]]; then
        ...
        sudo apt autoclean
        # Eventuell fehlende Pakete installieren wenn noch nicht vorhanden (Achtung:  Nur für DEBIAN-Linux !!!)
        ...
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" wget # wget installieren wenn noch nicht vorhanden
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" rpl # rpl installieren (Für deutsche Weboberfläche)
        clear # Bildschirm leeren
        ...
        sleep 3
        if ! command -v pihole >/dev/null 2>&1; then
        printf "\n❌ Fehler: Pi-hole konnte nicht installiert werden - Abbruch!\n\n"
        return 1 # Installation abbrechen
        fi
        # *************** Weboberfläche auf Deutsch einstellen  - Danke pimanDE ***********************************
        clear # Bildschirm leeren
        printf "\n\n"
        read -p "❓ Soll Pi-hole auf Deutsch übersetzt werden ? (ja/nein): " tconfirm
        if [[ "$tconfirm" == "ja" ]]; then
...
defaulttomlfile() {
 ...
 local param="$1"
   if [[ -z "$param" ]]; then
   printf "\n⚠️ Die Funktion 'defaulttomlfile' kann so nicht aufgerufen werden - Abbruch! \n\n"
    return 0
   fi
...
sleep 2
   ...
   if [[ -s "/etc/pihole/pihole.toml" ]]; then
     return 0
      ...
      printf "\n❌ Fehler: Die Datei /etc/pihole/pihole.toml konnte nicht angelegt werden - Abbruch!\n\n"
     return 1
   fi
else
 printf "\n❌ Fehler: ungültiger Parameter - Abbruch !\n\n"  
fi
}
      ...
      if [[ "$param" != "-ub" ]]; then
       clear # Bildschirm leeren
        while true; do
            ...
            printf "4) Cloudflare und OpenDNS\n"
            ...
            if [[ "$choice" =~ ^[1-8]$ ]]; then
                ...
                printf "\n❌ Ungültige Eingabe! Bitte wählen Sie eine Zahl von 1 bis 8.\n"
                sleep 3
            fi
        done
      else
        choice=8  # 'UNBOUND' Vorauswahl
      fi

        # ----- Zuweisung der DNS-Liste -----
        if [[ "$choice" == 8 ]]; then
            # Wenn UNBOUND: Variablen für externe Anbieter werden übersprungen
            if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
               dns_list='[ "127.0.0.1#5335", "::1#5335" ]' # IPv4 und IPv6
            else
               dns_list='[ "127.0.0.1#5335" ]' # nur IPv4
            fi
        else
            # Lokale Variablen NUR für externe Anbieter laden
        ...
        if [[ "$choice" == 8 ]]; then
         ...
         # Eintrag für die TOML vorbereiten
         ...
         sleep 3
         printf "\n⚙️ Die Pi-hole Optimierung ist abgeschlossen...\n\n"
...
installunbound() {
    ...
    # Vorab-Check: Ist Pi-hole installiert ?
    if ! command -v pihole >/dev/null 2>&1; then
    clear # Bildschirm leeren
    printf "\n\n⚠️ Achtung: Installieren Sie erst Pi-hole und wählen Sie als Upstream DNS-Server 'UNBOUND' !\n\n"
    printf "\n\n⌨️ Weiter mit beliebiger Taste...\n\n"
    read -n 1 -s -r
    piholeinstall # Pi-hole jetzt installieren
    return 1
    fi

    if dpkg -l | grep -q "^ii  unbound "; then
        printf "\n\n\nℹ️ 'UNBOUND' ist bereits auf diesem System installiert.\n\n"
        return 0
    fi
    ...
    local ub_conf="/etc/unbound/unbound.conf.d/pi-hole.conf"
        ...
        clear # Bildschirm leeren
        printf "\n\n🚀 Starte die Installation von 'UNBOUND'...\n\n"
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" unbound

        if dpkg -l | grep -q "^ii  unbound "; then

            printf "\n⚙️  Erstelle 'UNBOUND'-Konfiguration für Pi-hole...\n\n"
            # --- WICHTIG: Port-Konflikt mit Standard-Config vermeiden ---
...
remote-control:
    control-enable: yes
EOF"
            printf "\n🌐 Lade Root-Hints (Root-Server Liste) für 'UNBOUND' herunter...\n\n"
            # URL für die UNBOUND Server:
            sudo wget https://www.internic.net/domain/named.root -qO /var/lib/unbound/root.hints

            printf "\n🔧 Setze Berechtigungen und bereite Start vor...\n\n"
            ...
            printf "\n🔄 Starte den 'UNBOUND'-Dienst neu...\n\n"
            # Haupt-Konfiguration komplett leeren und nur auf den .d/ Ordner verweisen
            sudo bash -c 'echo "include-toplevel: \"/etc/unbound/unbound.conf.d/*.conf\"" > /etc/unbound/unbound.conf'
            # Den Socket stoppen und deaktivieren wenn vorhanden (der blockiert oft Port 53)
            ...
            # --- Pi-hole v6 AUTOMATISIERUNG ---
            printf "\n💉 Konfiguriere Pi-hole v6 Upstream-DNS auf 'UNBOUND'...\n\n"
            ...
            if command -v pihole-FTL >/dev/null 2>&1; then
                ...
                printf "✅ Pi-hole v6 wurde automatisch auf 'UNBOUND' umgestellt.\n"
            fi

            setunboundcronjob # Cronjob für die Aktualisierung der 'UNBOUND' Root-Hints

            # --- ABSCHLUSSMELDUNG ---
            printf "\n====================================================\n"
            printf "✅ UNBOUND ERFOLGREICH INSTALLIERT UND KONFIGURIERT\n"
            printf "====================================================\n"
            if [[ "$ipv6_do" == "yes" ]]; then
                printf "📍 IPv4 Adresse: 127.0.0.1#5335\n"
                printf "📍 IPv6 Adresse: [::1]#5335\n"
                printf "🛡️ DNSSEC:       Aktiviert\n"
            else
                printf "📍 IPv4 Adresse: 127.0.0.1#5335\n"
                printf "🛡️ DNSSEC:       Aktiviert\n"
            fi
            ...
            read -n 1 -s -r
            clear # Bildschirm leeren
            checkunbound # 'UNBOUND'-Installation prüfen
         else
            printf "\n❌ Fehler: Die Installation von 'UNBOUND' ist fehlgeschlagen !\n\n"
            return 1
        fi
}
...
setunboundcronjob() {
    ...
    cat <<-EOF > "$script_path"
#!/bin/bash
# Unbound Root-Hints aktualisieren
date > "$log_path" 2>&1
printf "🔄 Aktualisiere 'UNBOUND' Root-Hints...\n" >> "$log_path"
...
printf "✅ Aktualisierung der 'UNBOUND' Root-Hints abgeschlossen.\n" >> "$log_path"
EOF

    chmod +x "$script_path" # Skript ausführbar machen

    # Cronjob hinzufügen: Am 1. Tag im Januar (1) und Juli (7) um 03:30 Uhr
    local cron_job="30 3 1 1,7 * $script_path"
    (crontab -l 2>/dev/null | grep -vF "$script_path"; echo "$cron_job") | crontab -
    printf "\n📅 Cronjob erstellt: Die 'UNBOUND' Root-Hints werden alle 6 Monate (Jan & Juli) aktualisiert.\n"
}
...
checkunbound() {
  ...
  if ! command -v pihole >/dev/null 2>&1; then
   ...
   return 1
    ...
    if ! dpkg -l | grep -q "^ii  unbound "; then
      clear # Bildschirm leeren
      printf "\n\n⚠️ 'UNBOUND' ist noch NICHT installiert!\n\n"
      printf "\n⚠️ 'Achtung: Wenn 'UNBOUND' installiert wird dann läuft Pi-hole über diesen Dienst !\n\n\n"
      read -p "❓ Soll 'UNBOUND' jetzt installiert werden ? (ja/nein): " ubconfirm
      if [[ "$ubconfirm" == "ja" ]]; then
      upstreamdns -ub # UNBOUND installieren
      else
      printf "\n\n"
      return 1
      fi
         ...
         clear # Bildschirm leeren
         sudo unbound-control flush pi-hole.net > /dev/null 2>&1 # pi-hole.net aus dem cache löschen 
            # --- LIVE-TEST DER REKURSION ---
            printf "\n\n\n🔍 Teste DNS-Auflösung über 'UNBOUND (Vorher / Nachher)...\n"
            # 1. Test: Rekursion (Internet-Abfrage)
            printf "\n\n🔍 1. IPv4-Abfrage (Direkt aus dem Internet):\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @127.0.0.1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ FEHLER: 'UNBOUND' antwortet nicht auf Port 5335! (IPv4)\n"
            fi
            printf "====================================================\n"
            sleep 2 # Kurze Pause für die Optik
            # 2. Test: Caching (Lokale Abfrage)
            printf "\n\n🔍 2. IPv4-Abfrage aus dem 'UNBOUND'-Cache:\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @127.0.0.1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ FEHLER: Die IPv4 DNS-Abfrage über 'UNBOUND' ist fehlgeschlagen!\n"
            fi
            printf "====================================================\n\n\n"
            printf "\n🏁 'UNBOUND'-IPv4 Tests abgeschlossen. Weiter mit beliebiger Taste...\n"
            read -n 1 -s -r
            if ip -6 addr show 2>/dev/null | grep -q "scope global"; then
            clear # Bildschirm leeren
            printf "\n\n\n🔍 Teste IPv6 DNS-Auflösung über 'UNBOUND (Vorher / Nachher)...\n\n"
            sudo unbound-control flush pi-hole.net > /dev/null 2>&1 # pi-hole.net aus dem cache löschen
            # 1. Test: Rekursion (Internet-Abfrage)
            printf "\n\n🔍 1. IPv6-Abfrage (Direkt aus dem Internet):\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @::1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ FEHLER: 'UNBOUND' antwortet nicht auf Port 5335! (IPv6)\n"
            fi
             printf "====================================================\n\n"
            sleep 2 # Kurze Pause für die Optik
            # 2. Test: Caching (Lokale Abfrage)
            printf "\n\n🔍 2. IPv6-Abfrage aus dem 'UNBOUND'-Cache:\n"
            printf "====================================================\n"
            printf ";; Query on: pi-hole.net\n"
            if ! dig pi-hole.net -p 5335 @::1 | grep -E "Query time|SERVER:|WHEN:|MSG SIZE"; then
             printf "❌ FEHLER: Die IPv6 DNS-Abfrage über 'UNBOUND' ist fehlgeschlagen!\n"
            fi
            printf "====================================================\n\n\n"
            printf "\n🏁 'UNBOUND'-IPv6 Tests abgeschlossen. Weiter mit beliebiger Taste...\n"
            read -n 1 -s -r
            fi
            clear # Bildschirm leeren
            printf "\n\n\n"
...
setstaticip() {
    ...
    if [[ -f "$skip_file" ]]; then
        printf "\n\n⚠️ ACHTUNG: Der Benutzer hatte sich für dauerhaft für DHCP entschieden !\n\n"
    ...
    if [[ "$sip_antwort" == "ja" ]]; then
        ...
        printf "\n⌨️ Weiter mit beliebiger Taste...\n\n"
        read -n 1 -s -r
        printf "\n\n🚀 Neustart in 5 Sekunden, um auf die neuen IP-Adressen umzustellen - Bitte warten...\n\n"
        sleep 5
        sudo reboot
        exit 0
